...
import "./global.css";
import "./mcp-app.css";
import type { Customer, SegmentSummary, MetricName } from "./types";
import { SEGMENT_COLORS, METRIC_LABELS } from "./types";
import { generateCustomers, generateSegmentSummaries } from "./data-generator";
...
const sizeMetricSelect = document.getElementById("size-metric") as HTMLSelectElement;
...
function getMetricRange(customers: Customer[], metric: MetricName): { min: number; max: number } {
    ...
    options: {
      ...
      onClick: (_event, elements) => {
        if (elements.length > 0) {
          ...
          const point = dataset.data[element.index] as unknown as { customer: Customer };
...
function updateChart(): void {
  ...
  const scales = state.chart.options.scales as {
    x: { title: { text: string; color: string }; ticks: { color: string; callback: (value: number) => string }; grid: { color: string } };
    y: { title: { text: string; color: string }; ticks: { color: string; callback: (value: number) => string }; grid: { color: string } };
  };
...
function resetDetailPanel(): void {
  detailPanel.innerHTML = '<span class="detail-placeholder">Hover over a point to see details</span>';
}

// Initialize data from structuredContent in window
function initializeData(): void {
  // Check if data is provided via window (from tool result)
  const windowData = (window as { __TOOL_DATA__?: { customers: Customer[]; segments: SegmentSummary[] } }).__TOOL_DATA__;

  if (windowData?.customers && windowData?.segments) {
    state.customers = windowData.customers;
    state.segments = windowData.segments;
  } else {
    // Generate data locally for standalone testing
    state.customers = generateCustomers(250);
    state.segments = generateSegmentSummaries(state.customers);
  }
