import type { Customer, SegmentSummary, SegmentName } from "./types";
import { SEGMENT_COLORS, SEGMENTS } from "./types";

// Company name generation
const PREFIXES = [
  "Apex", "Nova", "Prime", "Vertex", "Atlas", "Quantum", "Summit", "Nexus",
  "Titan", "Pinnacle", "Zenith", "Vanguard", "Horizon", "Stellar", "Onyx",
  "Cobalt", "Vector", "Pulse", "Forge", "Spark",
];

const CORES = [
  "Tech", "Data", "Cloud", "Logic", "Sync", "Flow", "Core", "Net", "Soft",
  "Wave", "Link", "Mind", "Byte", "Grid", "Hub",
];
...
function generateCustomer(id: number, usedNames: Set<string>): Customer {
  ...
  return {
    ...
    annualRevenue: Math.round(
      generateClusteredValue(center.annualRevenue.min, center.annualRevenue.max)
    ),
    employeeCount: Math.round(
      generateClusteredValue(center.employeeCount.min, center.employeeCount.max)
    ),
    accountAge: Math.round(
      generateClusteredValue(center.accountAge.min, center.accountAge.max)
    ),
    engagementScore: Math.round(
      generateClusteredValue(center.engagementScore.min, center.engagementScore.max)
    ),
    supportTickets: Math.round(
      generateClusteredValue(center.supportTickets.min, center.supportTickets.max)
    ),
...
export function generateSegmentSummaries(customers: Customer[]): SegmentSummary[] {
